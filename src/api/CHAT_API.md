...
- 서버가 `chat.bot-email`/`chat.bot-nickname` 설정을 바탕으로 봇 계정을 조회하고 없으면 자동 생성하여, 사용자+봇 채팅방을 재사용/생성합니다.
...
- 봇 계정은 `chat.bot-email`/`chat.bot-nickname` 설정을 기반으로 서버가 자동 생성/재사용합니다.
...
1. **Environment 변수 설정**
   ...
   chatroom_id: [생성 후 입력]
   chat_bot_email: [선택] ai-bot@chat.local
